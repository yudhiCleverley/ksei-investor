...
public class LocaleUtil {
	...
	private String _getDisplayName(String language, String country, Locale locale, Set<String> duplicateLanguages) {
		...
		if (duplicateLanguages.contains(locale.getLanguage())) {
			displayName = StringUtil.appendParentheticalSuffix(language, country);
		} else {
	...
	private String _getLongDisplayName(Locale locale, Set<String> duplicateLanguages) {
		...
		return _getDisplayName(locale.getDisplayLanguage(locale), locale.getDisplayCountry(locale), locale,
				duplicateLanguages);
	}
	...
	private String _getShortDisplayName(Locale locale, Set<String> duplicateLanguages) {
		...
		return _getDisplayName(language, StringUtil.toUpperCase(locale.getCountry()), locale, duplicateLanguages);
	}
	...
	private void _setDefault(String userLanguage, String userCountry, String userVariant) {
		...
		} else if (Validator.isNotNull(userLanguage) && Validator.isNotNull(userCountry)
	...
	private String _toBCP47LanguageId(String languageId) {
		...
		return StringUtil.replace(languageId, CharPool.UNDERLINE, CharPool.MINUS);
	}
	...
	private String[] _toDisplayNames(Collection<Locale> locales, Locale locale) {
	...
	Logger _log = LoggerFactory.getLogger(LocaleUtil.class);
